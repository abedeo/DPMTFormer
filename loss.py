...
def compute_recon_loss_only(model_out: Dict[str, Any], cfg: Optional[LossConfig] = None) -> torch.Tensor:
    if cfg is None:
        cfg = LossConfig()
    ...
    rec_raw = torch.zeros((), device=rec_z.device, dtype=rec_z.dtype)
    has_raw = (
        cfg.use_raw_recon_loss
        and "recon_raw_patch_hat" in model_out
        and "target_rec_raw_patch" in model_out
    ...
    if has_raw:
        ...
        rec_raw = _weighted_mean(raw_elem, weight_z, eps=cfg.eps)

    loss_identity = torch.zeros((), device=rec_z.device, dtype=rec_z.dtype)
    rec_is_identity = model_out.get("rec_is_identity")
    if rec_is_identity is not None:
        id_mask = (rec_is_identity > 0).to(dtype=rec_z.dtype)
        if id_mask.sum() > 0:
            z_id = rec_elem.mean(dim=-1)
            z_id_loss = (z_id * id_mask).sum() / id_mask.sum().clamp_min(cfg.eps)
            if has_raw:
    ...
    return rec_z + float(cfg.raw_recon_weight) * rec_raw + float(cfg.identity_loss_weight) * loss_identity
